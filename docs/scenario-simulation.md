...
The server persists each proposed configuration as a `SCENARIO_SIMULATION` assessment before returning its ID. A scenario does not mutate the mission. The user must explicitly select it, record a decision, and—only after scenario approval—apply it. Deltas compare OrbitForge’s deterministic outputs only; they do not predict actual performance improvement, collision risk reduction, fuel use, or mission success.
